...
class VideoRemixerState():
    ...
    EFFECTS_VIEW_HINT = "V"
    EFFECTS_FADE_HINT = "F"
    EFFECTS_BLOCK_HINT = "B"
    EFFECTS_LENS_HINT = "L"
    EFFECTS_HINTS = [EFFECTS_VIEW_HINT, EFFECTS_FADE_HINT, EFFECTS_BLOCK_HINT, EFFECTS_LENS_HINT]
    UPSCALE_HINT = "U"
    RESIZE_STEP = "resize"
    ...
    def get_hint(self, scene_label, hint_type, allow_multiple=False):
        ...
        if scene_label:
            ...
            if hint:
                hints = self.split_hint(hint, allow_multiple)
    ...
    def purge_paths(self, path_list : list, keep_original=False, purged_path=None,
        ...
        use_last_path: True=use last path segemtn of purge path as root directory
        ...
        for path in paths_to_purge:
            ...
            if use_last_path:
                ...
                use_purged_path = os.path.join(use_purged_path, last_path)

            if keep_original:
                # _, last_path, _ = split_filepath(path)
                # copy_path = os.path.join(use_purged_path, last_path)
                copy_files(path, use_purged_path)
            else:
    ...
    def split_scene(self, scene_index, split_percent, keep_before=False, keep_after=False,
        ...
        if backup_scene:
            ...
            purge_root = self.purge_paths([scene_path], keep_original=True, use_last_path=True)
            if purge_root:
                self.project.copy_project_file(purge_root)
