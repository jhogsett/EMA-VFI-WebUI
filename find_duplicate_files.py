...
def main():
    ...
    parser.add_argument('--required', default="hash,bytes", type=str,
        help="Must-have match kinds for deduplication (default: 'hash,bytes')")
    parser.add_argument('--optional', default="modified,created,basename", type=str,
        help="Also must have a match in one of these kinds for deduplication (default: 'modified,created,basename')")
    parser.add_argument("--dupepath", default=None, type=str,
        help="If specified, duplicate files are moved to this path, preserving their directory structure (optional, default None)")
    ...
    FindDuplicateFiles(args.path, args.path2, args.wild, args.recursive, args.required, args.optional, args.dupepath, log.log).find()
...
class FindDuplicateFiles:
    ...
    """
    if dupepath exists, path2 must be set also
    then any duplicates of path1 found in path2 are moved (or copied) to dupepath with the same directory structure

    """

    def find(self) -> None:
        """Invoke the Find Duplicate Files feature"""
        wildcard = f"*.{self.wild}"
        files = []
        files2 = []
        if self.recursive:
            path = os.path.join(self.path, "**", wildcard)
        ...
        if files_info:
            ...
            with Mtqdm().open_bar(len(kinds), desc="Finding Duplicates") as bar:
                for kind in kinds:
                    ...
                    if dupes:
                        ...
                        with Mtqdm().open_bar(len(dupe_set), desc="Examining Duplicates") as bar2:
                            for dupe_info in dupe_set:
                                dupe_result = {}
                                dupe_result_infos = []
                                # with Mtqdm().open_bar(len(dupe_set), desc="Examining Files") as bar3:
            ...
            if result:
                ...
                with Mtqdm().open_bar(len(kind_keys1), desc="Finding Double Matches-1") as bar:
                    for kind1 in kind_keys1:
                        kind1_seen.append(kind1)
                        dupes1 = result[kind1]

                        kind_keys2 = [key for key in result.keys() if key not in kind1_seen]
                        with Mtqdm().open_bar(len(kind_keys2), desc="Finding Double Matches-2") as bar2:
                            for kind2 in kind_keys2:
                                kind2_seen.append(kind2)
                                dupes2 = result[kind2]

                                with Mtqdm().open_bar(len(dupes1), desc="Examining Duplicates") as bar3:
                                    for dupe in dupes1:
                ...
                # with Mtqdm().open_bar(len(kinds) * (len(kinds)-1) * (len(kinds)-2), desc="Finding Triple Matches") as bar:
